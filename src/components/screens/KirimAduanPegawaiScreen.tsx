// Kirim Aduan - Pegawai (2026-09-14). Pola sama persis KirimAduanScreen
// (versi Orang Tua), TAPI TANPA ChildSwitcher (tidak ada anak yang terkait)
// dan kategori tujuan berbeda (Kepala Sekolah/TU/Admin IT - unit sendiri,
// lihat backend routes/aduanPegawai.js).
import React, { useEffect, useState } from "react";
import { View, Text, ScrollView, Pressable, ActivityIndicator } from "react-native";
import { useSafeAreaInsets } from "react-native-safe-area-context";
import * as ImagePicker from "expo-image-picker";
import { Send, Paperclip, MessageSquareWarning } from "lucide-react-native";
...
export function KirimAduanPegawaiScreen() {
  const insets = useSafeAreaInsets();
  const colors = useThemeColors();
  const [loading, setLoading] = useState(true);
  const [riwayat, setRiwayat] = useState<AduanRow[]>([]);

  const [kategori, setKategori] = useState<"kepala_sekolah" | "tu" | "admin_it">("kepala_sekolah");
  ...
  async function load() {
    setLoading(true);
    const res = await api.aduanPegawaiMine();
  ...
  async function handleSubmit() {
    ...
    setBusy(true);
    setMessage(null);
    const res = await api.aduanPegawaiSubmit({ kategori, isi: isi.trim(), buktiFotoUri: foto?.uri, buktiFotoMime: foto?.mimeType });
    setBusy(false);
    ...
    <ScrollView className="flex-1 bg-background px-4 pt-5" contentContainerStyle={{ paddingBottom: 32 + insets.bottom, gap: 20 }}>
      <Card padding="lg">
        <View className="flex-row items-center gap-1.5 mb-1">
          ...
          <Text className="text-sm font-semibold text-foreground">Kirim Aduan</Text>
        </View>
